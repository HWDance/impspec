...
def main(seed, n, n_int, two_datasets = True): 
    ...
    """ Initialise model """
    model_EY = causalKLGP(Kernel_A = Kernel, 
                   Kernel_V = Kernel, 
                   dim_A = A.size()[1], 
                   dim_V = V[1].size()[1], 
                   samples = 10**5,
    ...
    """ Train model """
    model_EY.train(Y,A,V,niter,learn_rate, reg = reg, force_PD = True)

    """ Get CBO prior kernel """
    
    def mean(X):
        doA = X.reshape(len(X),1)
        return model_EY.post_mean(Y,A,V,doA,
                               reg = reg, 
                               average_doA = True, 
    ...
    def var(X):
        doA = X.reshape(len(X),1)
        EYdoX = mean(X)
        EY2doX = model_EY.post_mean(Y**2,A,V,doA,
                               reg = reg, 
                               average_doA = True, 
                               intervention_indices = [3]) 
